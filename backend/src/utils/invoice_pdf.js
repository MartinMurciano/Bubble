 ...
 * @param {Object} data - { factura, detalles, usuario, billing }
 */
export function generateInvoicePDF({ factura, detalles, usuario, billing = {} }) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor("#fff").fontSize(28).font("Helvetica-Bold").text("Bubble", 50, 28);
    doc.fontSize(10).text("FACTURA", doc.page.width - 130, 30, { width: 80, align: "right" })
    ...
    doc.font("Helvetica").fillColor(muted).fontSize(10)
      .text(`Nombre: ${billing.nombre || usuario.nombre} ${billing.apellido || usuario.apellido}`, 50, infoY + 16)
      .text(`Email: ${usuario.email}`, 50, infoY + 30)
      .text(`DNI: ${billing.dni || "-"}`, 50, infoY + 44)
      .text(`Direccion: ${billing.direccion || "-"}`, 50, infoY + 58);

    doc.fillColor(dark).fontSize(10).font("Helvetica-Bold").text("FECHA DE EMISION", 350, infoY);
    doc.font("Helvetica").fillColor(muted).fontSize(10).text(
    ...
    // LINEA SEPARADORA
    doc.moveTo(50, 215).lineTo(545, 215).strokeColor("#e1d5e0").stroke();
    ...
    detalles.forEach((d, idx) => {
      const rowH = 44;
      if (idx % 2 === 0) doc.rect(50, y, pageWidth, rowH).fill("#fafafa");
    ...
    doc.fillColor("#fff").fontSize(11).font("Helvetica-Bold").text("TOTAL", 380, y + 10);
    doc.fontSize(14)
      .text(`$${Number(factura.total).toLocaleString("es-AR")}`, 380, y + 10, { width: 155, align: "right" });

    y += 50;
    doc.fillColor(muted).fontSize(9).font("Helvetica").text("Metodo de pago: Tarjeta", 50, y);
